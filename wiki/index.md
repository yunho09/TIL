...
- [[크로스오리진-파일-다운로드-a-download-무시]] — 파일 주소가 페이지와 다른 도메인이면 `<a download>`가 무시돼 새 탭으로 열리거나 파일명이 key로 나오는 원인, CORS GET 허용(fetch+Blob) vs `Content-Disposition` 헤더 두 해결 경로, 비공개 버킷은 presigned URL 필요

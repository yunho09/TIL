...
## 2026-09-10 19:58 — ingest (Claude Code 세션 자동 캡처)
- 원본: Claude Code 세션 자동 캡처 (/home/yunho)
- 생성: [[GPU-유휴클럭-램프업-지연-프레임드랍]] (신규, `환경/리눅스-데스크톱/`)
- 비고: 노트북 프레임 드랍 진단 세션. 두 차례 오진단(컴포지터/패널 60Hz 하락, Discord `--disable-gpu`)을 모두 실측으로 반증한 뒤 "데스크톱 이동할 때만" 버벅인다는 사용자 특정으로 GNOME Shell 애니메이션 쪽으로 좁혔고, GPU 유휴 클럭 400MHz+램프업 ~1.5초 vs 애니메이션 0.2~0.4초라는 확정 원인에 도달했다. 재사용 가치가 있는 진단 기법(presentation-time 프로토콜 실측, 120Hz/60Hz 시각 비교자로 체감-측정 교차검증, act_freq 시계열 샘플링)과 `min_freq` sysfs 해결책+배터리 트레이드오프를 페이지에 남겼다. XWayland 소수배율 2배 버퍼+프레임 페이싱 문제는 이번 증상의 주범은 아니지만 별개 사실로 페이지 안에 미해결로 기록. "오늘 갑자기" 시작된 계기는 끝내 못 찾았고 페이지에도 미해결로 명시했다. 세션 도중의 시행착오(pkill이 자기 셸을 죽인 것, rAF가 숨은 탭에서 안 도는 버그, 샘플 부족으로 재측정한 것 등)는 결론에 안 남긴 과정이라 제외했다.

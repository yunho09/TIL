...
## 변형: 수정 폼 초기값으로 placeholder를 쓰면 저장 시 서버 값이 덮어써짐 (2026-09-19 추가)

- 운영시간 수정 폼이 조회가 끝나기 전 `placeholderData`로 기본 운영시간을 보여줬고, 그 사이 저장하면 **서버의 실제 값이 기본값으로 바뀌었다**(ToyVillage-Admin-FE, 7월 사례).
- 원인은 같은 뿌리: placeholder는 화면용 임시값인데 폼이 이를 편집 가능한 원본처럼 초기값으로 썼다.
- 해결: 조회가 끝나기 전(`isPending`)에는 폼을 그리지 않는다. "빨리 보여주기"와 "편집 가능한 원본"은 다르게 다룬다.
- 같은 화면에서 두 조회(예: 휴관일·운영시간)를 한 상태로 묶으면 한쪽 실패가 다른 쪽 폼까지 없앤다 → 쿼리 상태를 분리한다.
